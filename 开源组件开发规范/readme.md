...
- [开源项目开发规范](#开源项目开发规范)
	...
	- [2. 语言化版本](#2-语言化版本)
		- [2-1 摘要](#2-1-摘要)
		- [2-2 简介](#2-2-简介)
		- [2-3 语义化版本控制规范（SemVer）](#2-3-语义化版本控制规范semver)
		- [2-4 为什么要使用语义化的版本控制](#2-4-为什么要使用语义化的版本控制)
		- [2-5 FAQ](#2-5-faq)
			- [2-5-1 在 0.y.z 初始开发阶段，我该如何进行版本控制](#2-5-1-在-0yz-初始开发阶段我该如何进行版本控制)
			- [2-5-2 如何判断发布 1.0.0 版本的时机](#2-5-2-如何判断发布-100-版本的时机)
			- [2-5-3 这不会阻碍快速开发和迭代吗](#2-5-3-这不会阻碍快速开发和迭代吗)
			- [2-5-4 对于公共 API，若即使是最小但不向下兼容的改变都需要产生新的主版本号，岂不是很快就达到 42.0.0 版](#2-5-4-对于公共-api若即使是最小但不向下兼容的改变都需要产生新的主版本号岂不是很快就达到-4200-版)
			- [2-5-5 为整个公共 API 写文件太费事了](#2-5-5-为整个公共-api-写文件太费事了)
			- [2-5-6 万一不小心把一个不兼容的改版当成了次版本号发行了该怎么办](#2-5-6-万一不小心把一个不兼容的改版当成了次版本号发行了该怎么办)
			- [2-5-7 如果我更新了自己的依赖但没有改变公共 API 该怎么办](#2-5-7-如果我更新了自己的依赖但没有改变公共-api-该怎么办)
			- [2-5-8 如果我变更了公共 API 但无意中未遵循版本号的改动怎么办呢？（意即在修订等级的发布中，误将重大且不兼容的改变加到代码之中）](#2-5-8-如果我变更了公共-api-但无意中未遵循版本号的改动怎么办呢意即在修订等级的发布中误将重大且不兼容的改变加到代码之中)
			- [2-5-9 我该如何处理即将弃用的功能](#2-5-9-我该如何处理即将弃用的功能)
			- [2-5-10 语义化版本对于版本的字串长度是否有限制呢](#2-5-10-语义化版本对于版本的字串长度是否有限制呢)
		- [2-6 关于](#2-6-关于)
		- [2-7 许可证](#2-7-许可证)
	- [3. 最佳实践](#3-最佳实践)
...
**本章以下内容来源**：`https://github.com/semver/semver/blob/master/semver.md`
...
#### 2-5-1 在 0.y.z 初始开发阶段，我该如何进行版本控制

最简单的做法是以 0.1.0 作为你的初始化开发版本，并在后续的每次发行时递增次版本号。

#### 2-5-2 如何判断发布 1.0.0 版本的时机
...
#### 2-5-4 对于公共 API，若即使是最小但不向下兼容的改变都需要产生新的主版本号，岂不是很快就达到 42.0.0 版

这是开发的责任感和前瞻性的问题。不兼容的改变不应该轻易被加入到有许多依赖代码的软件中。升级所付出的代价可能是巨大的。要递增主版本号来发行不兼容的改版，意味着你必须为这些改变所带来的影响深思熟虑，并且评估所涉及的成本及效益比。

#### 2-5-5 为整个公共 API 写文件太费事了
...
#### 2-5-6 万一不小心把一个不兼容的改版当成了次版本号发行了该怎么办
...
#### 2-5-7 如果我更新了自己的依赖但没有改变公共 API 该怎么办
...
#### 2-5-8 如果我变更了公共 API 但无意中未遵循版本号的改动怎么办呢？（意即在修订等级的发布中，误将重大且不兼容的改变加到代码之中）
